    ...
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            if len(shape) == 2:
                ...
                form = 'csr' if shape[0] * shape[2] < shape[1] * shape[3] else 'csc'
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Construct matrix
            if 'conv' in name:
                mat = matrix((data, indices, indptr), [shape[0]*shape[2], shape[1]*shape[3]])
                # Insert to model
                param.data = torch.from_numpy(mat.toarray().reshape(shape[0],shape[1],shape[2],shape[3])).to(dev)
            else:
